<template>
  <div class="h-screen w-full flex relative">
    ...
    <div class="flex-1 flex flex-col py-4 overflow-hidden pt-24 overflow-y-auto relative z-10">
      ...
      <div v-if="!isAuthenticated" class="login-box flex flex-col items-center text-center p-6">
        ...
        <div class="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          ...
          >
            <div 
              ...
              :style="{ backgroundImage: `url(src/assets/img/${normalizeName(area.nombre)}.webp)` }"
            ></div>
...
const contenidos = ref([]) // Propiedad definida para evitar warning
const gradoNames = ref([]) // Cambiar a un array de objetos con id y nombre
const areaNames = ref([]) // Definir areaNames como un array de objetos con id y nombre
const { isAuthenticated, user, checkAuth } = useAuth() // Obtener el estado real de autenticación y el usuario
...
onMounted(async () => {
  ...
  if (!isAuthChecked) {
    ...
    await fetchGrados() // Llamar a la función para cargar los grados
    await fetchAreas() // Llamar a la función para cargar las áreas
  }
  await loadSlideshowImages(); // Cargar imágenes dinámicamente
